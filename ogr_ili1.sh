...
echo "*** Port-Valais ***"
./gdal-build/bin/ogr2ogr --config OGR_STROKE_CURVE TRUE -a_srs "EPSG:2056" -gt 20000 -append -f PostgreSQL "PG:dbname=cadastre active_schema=valais" data/port-valais.itf,bdco.imd
echo "*** VD-1 ***"      
./gdal-build/bin/ogr2ogr --config OGR_STROKE_CURVE TRUE -a_srs "EPSG:2056" -gt 20000 -append -f PostgreSQL "PG:dbname=cadastre active_schema=vaud" data/vd-1.itf,bdco.imd
echo "*** VD-2 ***"      
./gdal-build/bin/ogr2ogr --config OGR_STROKE_CURVE TRUE -a_srs "EPSG:2056" -gt 20000 -append -f PostgreSQL "PG:dbname=cadastre active_schema=vaud" data/vd-2.itf,bdco.imd
